...
from config import text_default_settings_discop
...
cdef CySingleEncodeStepOutput cy_encode_step(list indices, list probs, string message_bits):
    ...
    while not is_leaf(node_ptr):  # non-leaf node
        ...
        if not msg_exhausted_flag and (len_message_bits <= n_bits):
            print('[*] Secret message embedding complete!')
...
def encode(model, tokenizer, context, message_bits, settings, bint verbose = False, string tqdm_desc = b'Encoding:'):
...
def decode(model, context, list stego, settings, bint verbose = False, string tqdm_desc = b'Decoding:'):
...
def encode_text(model, tokenizer, message_bits, context, settings = text_default_settings_discop):
...
def decode_text(model, tokenizer, list stego, context, settings = text_default_settings_discop):
    # tokenizer = get_tokenizer(settings)
    # model = get_model(settings)
    context = tokenizer(context, return_tensors='pt', max_length=1024, truncation=True)['input_ids'].to(settings.device)
